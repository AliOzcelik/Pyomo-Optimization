...
overtime_pay = {1:120*40, 2:140*60}
mdl.V = pyo.Param(mdl.I, initialize=overtime_pay, doc="overtime work pay to workers")
...
mdl.O = pyo.Var(mdl.I, within=pyo.NonNegativeReals, doc="amount of overtime work")
mdl.H = pyo.Var(mdl.I, within=pyo.NonNegativeReals, doc="amount of finishing days after overtime work")
mdl.d_plus = pyo.Var(mdl.I, within=pyo.NonNegativeReals, doc="positive deviation number of days exceeding the day limit of projects")
mdl.d_minus = pyo.Var(mdl.I, within=pyo.NonNegativeReals, doc="negative deviation number of days exceeding the day limit of projects")
...
def h1_constraint(mdl):
    return mdl.H[1] == mdl.A - mdl.O[1]
mdl.h1_constraint = pyo.Constraint(rule=h1_constraint, doc="new finishing time of project A after overtime work")

def h2_constraint(mdl):
    return mdl.H[2] == mdl.B - mdl.O[2]
mdl.h2_constraint = pyo.Constraint(rule=h2_constraint, doc="new finishing time of project B after overtime work")

def new_year_constraint(mdl, i):
    return mdl.H[i] == mdl.G[i] + mdl.d_plus[i] - mdl.d_minus[i]
mdl.new_year_constraint = pyo.Constraint(mdl.I, rule=new_year_constraint,
                                          doc="determining the deviations from the limited time after the new finishing time has been calculated")

def year_constraint(mdl, i):
    return mdl.H[i] == mdl.x[i] + mdl.y[i]
mdl.year_constraint = pyo.Constraint(mdl.I, rule=year_constraint, doc="total number of days in projects")

def overwork_constraint1(mdl):
    return mdl.O[1] <= (mdl.A/2)
mdl.overwork_constraint1 = pyo.Constraint(rule=overwork_constraint1, doc="overtimework can't exceed the half of the normal finishing time")

def overwork_constraint2(mdl,i):
    return mdl.O[2] <= (mdl.B/2)
mdl.overwork_constraint2 = pyo.Constraint(rule=overwork_constraint2, doc="overtimework can't exceed the half of the normal finishing time")

def obj_func(mdl, i):
    return sum(mdl.W[i]*mdl.x[i]*mdl.P[i] + mdl.W[i]*mdl.y[i]*mdl.U[i] + mdl.V[i]*mdl.O[i] + mdl.Z[i]*mdl.d_plus[i]  for i in mdl.I)
...
variable_data = {(i, v.name): pyo.value(v) for i,v in mdl.O.items()}
optimal_overtime_work = pd.DataFrame.from_dict(variable_data, orient="index", columns=["variable value of Overtime Work"])
optimal_overtime_work.to_excel(r'C:\Users\HP\Desktop\dersler\IE251\Case 2\Son\Overtime.xlsx', sheet_name='Overtime Work')

variable_data = {(i, v.name): pyo.value(v) for i,v in mdl.H.items()}
optimal_new_finishing_time = pd.DataFrame.from_dict(variable_data, orient="index", columns=["variable value of New durations"])
optimal_new_finishing_time.to_excel(r'C:\Users\HP\Desktop\dersler\IE251\Case 2\Son\New_Finishing_days.xlsx', sheet_name='New Finishing Times')
...
Reduced_Costs_print.to_excel(r'C:\Users\HP\Desktop\dersler\IE251\Case 2\Son\ReducedCostsPart2_1.xlsx', sheet_name='ReducedCosts')
...
Shadow_Prices_print = pd.concat([pd.Series(d,name=name)
          ...
          axis='columns')
Shadow_Prices_print.to_excel(r'C:\Users\HP\Desktop\dersler\IE251\Case 2\Son\ShadowPricesPart2_1.xlsx', sheet_name='ShadowPrices')
